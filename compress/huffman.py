...
__all__ = ['huffman_encode']
__ref__ = "https://github.com/mightydeveloper/Deep-Compression-PyTorch/blob/master/net/huffmancoding.py"
...
def huffman_encode(arr, prefix, save_dir='./'):
    ...
    datasize = dump(data_encoding, directory/f'{prefix}.hf.dat')

    # Dump codebook (huffman tree)
    codebook_encoding = encode_huffman_tree(root, dtype)
    treesize = dump(codebook_encoding, directory/f'{prefix}.hfcb.dat')
...
def encode_huffman_tree(root, dtype):
    ...
    """
    converter = {'float32':float2bitstr, 'int32':int2bitstr, 'uint8': byte2bitstr}
    code_list = []
